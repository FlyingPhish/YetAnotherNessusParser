...
def process_file(
    ...
    parsing, optional consolidation (Nessus/Burp), optional API formatting (Nessus/Burp),
    and optional flat JSON output (Nmap only).
    
    Args:
        input_file: Path to input file
        file_type: File type ('auto', 'nessus', 'nmap', 'burp')
        port_status: Port status filter for Nmap ('all', 'open', 'closed', 'filtered')
        consolidate: Whether to apply consolidation rules (Nessus/Burp)
        api_format: Whether to format for API consumption (Nessus/Burp, requires consolidate=True)
        rules_file: Path to custom consolidation rules file (Nessus/Burp)
        entity_limit: Maximum number of affected entities per API finding (Nessus/Burp)
    ...
    Raises:
        ...
        ConsolidationError: If consolidation fails (Nessus/Burp)
        FormatterError: If API formatting fails (Nessus/Burp)
    ...
    Examples:
        ...
        Burp basic parsing:
            ...
            >>> burp_data = results['parsed']
            >>> print(f"Found {burp_data['stats']['vulnerabilities']['total']} vulnerabilities")
    ...
    if file_type == "nessus":
        ...
        parser = BurpParser(input_file)
        parsed_data = parser.parse()
        results['parsed'] = parsed_data
        results['file_type'] = 'burp'
        
        # Future: consolidation and API formatting for Burp
        # For now, just basic parsing to establish foundation
        if consolidate:
            logger.warning("Burp consolidation not yet implemented - returning parsed data only")
        if api_format:
            logger.warning("Burp API formatting not yet implemented - returning parsed data only")
